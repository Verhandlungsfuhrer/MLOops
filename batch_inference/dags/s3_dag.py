...
def upload_data(data_name, s3_address, s3_bucket):
    import minio
    client = minio.Minio(s3_address, secure=False, access_key="miniouser", secret_key="minioser")
...
with DAG("s3_uploader", default_args=default_args, schedule_interval="0 12 * * 1-5", start_date=days_ago(1)):
    PythonVirtualenvOperator(task_id="upload_data", python_callable=upload_data,
